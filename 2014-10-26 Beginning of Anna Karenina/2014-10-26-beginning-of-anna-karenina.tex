...
The relativizer \ayr{si} \fw{si} exhibits secondary marking for the referent's 
syntactic role in the relative clause: in the case of this passage, the referent
fulfills the role of a patient in the relative clause. When the case agreement 
with the referent's case in the matrix clause is dropped (as is the case here),\footnote{
This is permissible whenever there is no material between the head of the 
relative clause and the relativizer.} the vowel of the secondary case marker is 
often found being prolonged in order to indicate the dropping because otherwise,
both forms would be homophonous:
...
\begin{exe}
	...
	\ex \begin{tabbing}
		...
		\= \enskip + \enskip \ayr{/ymF} \fw{-yam} \qq{\mor{-DAT}}
		\= \hspace{7em}
		\= \enskip $\Rightarrow$ \enskip \ayr{siymF} \fw{siyam} \qq{\mor{REL-DAT}} \\
	    \ayr{si} \fw{si} \qq{\mor{REL}}
		\> \enskip + \enskip \_
		\> \enskip + \enskip \ayr{ymF} \fw{yam} \qq{\mor{DAT}}
		\> \enskip $\Rightarrow$ \enskip \ayr{siyaamF} \fw{siyām} \qq{\mor{REL-Ø-DAT}}
	\end{tabbing}
	
	\ex \begin{tabbing}
	    \ayr{si} \fw{si} \qq{\mor{REL}}
		\= \enskip + \enskip \ayr{/Is} \fw{-isa} \qq{\mor{-CAU}} \hspace{0.25em}
		\= \hspace{7em}
		\= \enskip $\Rightarrow$ \enskip \ayr{sis} \fw{sisa} \qq{\mor{REL-CAU}} \\
	    \ayr{si} \fw{si} \qq{\mor{REL}}
		\> \enskip + \enskip \_
		\> \enskip + \enskip \ayr{saa} \fw{sā} \qq{\mor{CAU}}
		\> \enskip $\Rightarrow$ \enskip \ayr{sisaa} \fw{sisā} \qq{\mor{REL-Ø-CAU}}
	\end{tabbing}
    \end{xlist}
\end{exe}
...
prefixed form, i.e. the form that is used with names.
...
the dictionary, but they are nouns regularly derived from verbs, in so far 
transparent and not in desperate need of a new dictionary entry.
