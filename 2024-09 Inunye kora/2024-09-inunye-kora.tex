...
% saa til—\\
% —yNF niv\\
% vn dimjyeymF.			\tab \textit{tiMkaanFreNF}\\
% y nsYyNF vaasF lito		\tab \textit{tiMkaanFreNF}\\
% n mvj					\tab \textit{tiMkaanFreNF}\\
% lesyNF.\\
% ANF sgonFy EnFy maaj	\tab \textit{tiMkaanFreNF}\\
% ANF suNFy mimaanFlej.	\tab \textit{tiMkaanFreNF}\\
% nj Ed/—					\tab \textit{tiMkaanFreNF}\\
% d/naa.
...
break in the middle, leaving the topicalization of \textquote{eyes} intact by
using a regular causative construction. Since the Ayeri translation only
amounts to three words, another question is where to divide the lines. I
decided to make the cut according to the number of syllables, so right inside
the word \rayr{tilyNF}{tilayang} before the heavy syllable \rayr{/yNF}{-yang}.
This changes the distribution of words per line from 2~+~3 in English to 3~+~3
in Ayeri, though it works because Yorke is rather drawing out \textquote{your
eyes.}
...
the eyes, \xayr{vn}{vana}{your}, falls victim to keeping the line length
similar to the English text. The Ayeri verb \rayr{til/}{tila-} can mean
`change' when used intransitively, so the sentence is more or less sensible on
its own, but the context as to whose gaze and the how or why is similarly lost.
...
\ex \label{ex:2d}
	\gll Ya nasyyang vās lito na Mavay --- nay lesayang. \\
