...
contains an anadiplosis again: \textquote{Yeah, I---}. Since Ayeri works
differently from English and the full clause begins with the agent-topic marker
\rayr{ANF}{ang}, I translated this bit as \rayr{maaj ANF}{māy ang} and
\rayr{ANF}{ang} in the repetition, respectively.
...
This, in turn is what the similar verb \xayr{gireMdF/}{girend-}{arrive} is for,
which is one syllable too long in this context, however. So \rayr{hrF/}{har-}
it was after all, but with the destination \xayr{AvnFy}{avanya}{on the ground}
in the locative case to distinguish it from the victim, in the patient case, of
a blow with one's fist, for example.
