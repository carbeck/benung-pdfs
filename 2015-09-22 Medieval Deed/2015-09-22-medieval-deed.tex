...
Since I have come up with ideas for another language a while ago (the Ayeri 
exonym for it being \xayr{turyi}{Turayi}{Hillandic}), however, I couldn't help 
thinking about it, so how about this: % \ayr{AAsFti/nF Ñ paagF k stF!} \fw{Āsti-n 
% na Pāg ka sat!} (name-\Def{} \Def{} Lord of with!) `With the name of the Lord!'

\ex[exno=1′] \begingl
	\glpreamble Āsti-n na Pāg ka sat! // %{\Tagati AAsFti/nF Ñ paagF k stF!} //
	\gla Āsti @ -n na Pāg ka sat! //
	\glb name -\Def{} \Def{} Lord of with! //
	\glft \enquote{With the name of the Lord!} //
\endgl
\xe

Turayi in its modern version is supposed to be some kind of anti-Ayeri in being 
fairly isolating with modifier-head word order, which licenses postpositions. 
As a little twist, however, articles go into the so-called Wackernagel position, 
that is, if nothing occupies this syntactic position, they go into the 
second position of the \Np{}. In Turayi, you would thus get 
\textsc{n}~\textsc{det} if no adjectives are involved, but 
\textsc{adj}~\textsc{det}~\textsc{n} if there are. There would, however, be an 
exception for possessive phrases, in which the word order 
\textsc{det}\fakesubscript{i}~\textsc{n}~\_\fakesubscript{i}~\emph{of} 
would be preferred instead of \textsc{n det}~\emph{of} as one would 
otherwise expect. I will have to think about whether postpositions in general 
cause movement in spite of different syntactic levels.
...
\pex
	...
	\a \begingl
		...
		@ tahang @ -ley niva @ -ye @ -eri soyang tang @ -ye @ -eri 
			tan,~… //
		\glb \AgtT{} be.supposed know -\Tpl{}.\M{} people =all \Rel{} 
	...
	\a \begingl
		\gla ya mya veh @ -oy @ -ya ang= @ Sikontendo Biratayati tiga 
			...
			-isa @ -ong @ -ara yās. //
		\glb \LocT{} be.supposed build -\Neg{} -\Tsg{}.\M{} \Aarg{}= 
			Chunrat {der Hafener} honorable inside this= garden.\Top{} half 
...
genre and the way basically all deeds in the \citetitle{CAO} begin, with minor 
variations.
...
\begin{tabu} to \textwidth {X[40] X[60] X[40] X[60]}
	...
	Chunrat
		...
		& \xayr{sikoNF}{sikong}{advice}, \newline
			\xayr{teMdo}{tendo}{courageous} \\ [0.5\baselineskip]
...
\ex \begingl
	...
	\glb that -\Aarg{}.\Inan{} certain, \AgtT{} go -\Tsg{}.\M{} \Top{}= 
		Chunrat {der Hafener} honorable there until and \AgtT{} want build 
...
\pex
	...
	\a \begingl
		...
		\glb \AgtT{} want build -\Tsg{}.\M{}.\Top{} fence 
			-\Parg{}.\Inan{} although be.supposed such= build 
			-\Neg{} -\Tsg{}.\M{}.\Aarg{}~… //
...
I was a little stumped at first about how to translate that \fw{do} `when' 
there, since Ayeri uses its equivalent question word \xayr{sitdj}{sitaday}{when} 
only in questions but usually not as a relative pronoun. If my mind is not 
playing tricks at me, the sentence should have a headless relative clause in 
both German and English here, leaving out something like \fw{ze der zît} or 
`at the time', respectively. This realization brought me to remember that I 
could just translate it as that, which is how we get \ayr{tdjy si}
\fw{tadayya si} here.
...
% Deſ laugent im der Hafener, daz er mit im alſo iht kauft hete. Dar vber wart 
% erteilt, daz der Hafener bereite, daz er mit im alſo iht kauft hete, in 
% bezivgte danne her Vlrich, daz ez alſo wæ̂re.
...
\ex \begingl
	...
	\gla Ang no da= @ subr @ -oy @ -ya Ø= @ Hasanjan tiga kaytan @ -ley yana 
		...
		-yāng. //
	\glb \AgtT{} want thus= cease -\Neg{} -\Tsg{}.\M{} \Top{}= Ulrich 
...
reasonable guess. For \fw{Chonrat} and \fw{Ulrich}, see 
page~\pageref{tab:names1} above. While \fw{Chonrat}, \fw{Ulrich} and 
\fw{Liupolt} simply can be looked up in a dictionary,\footnote{Such as 
\textcite{duden:vornamen}, for example, though Wiktionary said the same things 
in all cases.} the other three names I could only guess at indirectly by looking 
up similar modern names.
...
\fw{Siegfried}, \fw{Si(e)gmar}, \fw{Siegmund}, where the \fw{Sieg-} part is from 
\Ohg{} \fw{sigu} `victory'. For \fw{Volkwin}, I assumed a relation to \Nhg{} 
\fw{Volk} `people' (\Mhg{} \norm{volc}, \Ohg{} \fw{folc} `horde, people'), and 
the \fw{-win} part may well be related to the \fw{-win} in \fw{Erwin}, where it 
is from \Ohg{} \fw{wini} `friend'. \autoref{tab:names2} gives a summary of 
these names and their calques.
...
\begin{tabu} to \textwidth {X[40] X[60] X[40] X[60]}
	...
	Sibot
		...
		& \xayr{bjhi}{bayhi}{ruler}, \newline
			\xayr{siktu}{sikatu}{victorious} \\ [0.5\baselineskip]
...
A problem with two-part Germanic given names is certainly that when calquing 
them, they often become at least twice as long since nouns in Ayeri are very 
commonly disyllabic already.
